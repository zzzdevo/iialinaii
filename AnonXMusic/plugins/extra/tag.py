            ...
            async for member in client.get_chat_members(chat_id):
               if member.status == ChatMemberStatus.OWNER:
                 ...
                 if m.photo:
                       ...
                       return await message.reply_photo(photo, caption=f"<b>✧ ¦زانیاری خاوەن گرووپ \n\n ✧ ¦ ناو ← {m.first_name} \n ✧ ¦ یوزەر ← @{m.username} \n ✧ ¦ بایۆ ← {m.bio}</b>",reply_markup=key)
                 else:
...
async def sticker_image(client: Client, message: Message):
    ...
    if not reply:
        return await message.reply("<b>ڕپلەی ستیکەر بکە</b>")
    if not reply.sticker:
        return await message.reply("<b>ڕپلەی ستیکەر بکە</b>")
    m = await message.reply("<b>کەمێك چاوەڕێبە . .</b>")
...
async def vgdg(client: Client, message: Message):
    await message.reply_text(
        f"""<b>•⎆┊ ناوت 🔥♥»»  {message.from_user.mention()}</b>""") 
...
async def nummmm(client: app, message):
  if message.chat.id in array:
     return await message.reply_text(f"<b>•⎆┊تاگکردن دەستی پێکرد♥\n\n لەلایەن ← ✧ ¦{message.from_user.mention}•</b>")
  dev = (OWNER_ID)
  ...
  if not chek.status in  [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
    await message.reply(f"<b>•⎆┊ببورە تۆ ئەدمین نیت🗿 {message.from_user.mention}•</b>")
    return
  await message.reply_text(f"<b>•⎆┊تاگکردن دەستی پێکرد♥🪐 \n\n لەلایەن ← {rotba}✧ ¦{message.from_user.mention} \n\nبۆ کۆتایی هێنانی تاگ بنووسە وەستانی تاگ یان /cancel ♥🧩•</b>")
  i = 0
  txt = ""
  zz = message.text
  if message.photo:
...
async def stop(client, message):
  ...
  if not chek.status in  [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
    ...
    return
  if message.chat.id not in array:
     await message.reply(f"<b>•⎆┊تاگکردن وەستاوە ئەزیزم {message.from_user.mention}•</b>")
     return 
  if message.chat.id in array:
    array.remove(message.chat.id)
    await message.reply(f"<b>•⎆┊تاگکردن وەستێنرا \n\n✧ ¦ لەلایەن ← {rotba} {message.from_user.mention}•</b>")
    return
